/**
 * cart.js — estado del carrito, cálculos de totales, drawer y vista
 * /cart (relocado de js/cart.js en la reorganización arquitectónica).
 * El carrito NUNCA descuenta stock definitivo (ver
 * js/services/inventoryService.js) — solo referencia productId/
 * variantId/qty; el precio y el stock siempre se resuelven en vivo
 * contra Store.state.products (Supabase).
 *
 * El carrito en sí sigue siendo local (Storage — ver informe de
 * arquitectura, "Fuera de alcance": RLS por dueño exige Auth real de
 * cliente). Envío/cupones sí vienen de Supabase vía
 * Store.state.shippingConfig/Store.state.coupons.
 */
const CartModule = (() => {
  function shippingCfg() {
    // Fallback defensivo: null solo ocurre si Supabase todavía no
    // respondió o si un admin nunca guardó /admin/shipping — nunca se
    // inventa un valor de envío "verdadero", se usa 0 para no romper
    // el cálculo mientras tanto.
    return Store.state.shippingConfig || { standardShippingCost: 0, expressShippingCost: 0, freeShippingThreshold: 0 };
  }
  ...
  function totals() {
    ...
    if (freeShip || (cfg.freeShippingThreshold > 0 && subtotal - discount >= cfg.freeShippingThreshold)) shipping = 0;
  ...
  function renderDrawer() {
    ...
    const threshold = shippingCfg().freeShippingThreshold;
    const pct = threshold > 0 ? Utils.clamp(100 - (remaining / threshold) * 100, 0, 100) : 100;
  ...
  return { items, addItem, updateQty, removeItem, clear, totals, freeShippingRemaining,
    applyCoupon, removeCoupon, crossSell, comboSuggestion, renderDrawer, renderCartPage, shippingCfg };
})();
