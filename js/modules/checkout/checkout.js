/**
 * checkout.js — flujo de checkout en 4 pasos (relocado de
 * js/checkout.js en la reorganización arquitectónica). No obliga a
 * crear cuenta (permite "comprar como invitado") y simula el pago
 * (ningún dato de pago real se procesa ni se transmite).
 *
 * El pedido sigue siendo local (Storage — ver informe de arquitectura,
 * "Fuera de alcance": RLS por dueño + falta un RPC de creación de
 * pedido en Supabase). Los montos que arma OrdersModule.createOrder()
 * siguen siendo números en COP puros, sin ninguna conversión de
 * moneda — la preferencia COP/USD es solo presentación (ver
 * CurrencyService), nunca toca lo que se guarda acá.
 */
const CheckoutModule = (() => {
  ...
  async function resetState() {
    ...
    const addr = user ? await AuthModule.getDefaultAddress() : null;
    data = {
      guest: !user,
  ...
  function summaryCard() {
    ...
    return `<div class="summary-card">
      ...
      <div class="totals-row"><span>Envío</span><span class="mono">${data.shippingMethod === 'express' ? Utils.formatMoney(CartModule.shippingCfg().expressShippingCost) : (t.shipping === 0 ? 'Gratis' : Utils.formatMoney(t.shipping))}</span></div>
  ...
  async function renderShippingStep() {
    ...
    const savedAddresses = AuthModule.isLoggedIn() ? await AuthModule.listAddresses() : [];
    Utils.qs('#stepContent').innerHTML = `
      <div class="admin-panel">
  ...
  function processPayment() {
    ...
    setTimeout(async () => {
      ...
      if (AuthModule.isLoggedIn() && data.saveAddress) {
        // Fire-and-forget deliberado: es una conveniencia (guardar la
        // dirección para la próxima compra), no debe bloquear ni
        // hacer fallar la confirmación del pedido si falla.
        ...
        }).catch((err) => console.warn('[CheckoutModule] no se pudo guardar la dirección:', err.message));
      }
  ...
  function renderConfirmationStep() {
    ...
    if (createBtn) createBtn.addEventListener('click', () => {
      ...
      Utils.qs('#btnDoCreate').addEventListener('click', async () => {
        const pass = Utils.qs('#newPass').value;
        if (pass.length < 6) { Notify.error('La contraseña debe tener al menos 6 caracteres.'); return; }
        const btn = Utils.qs('#btnDoCreate'); btn.disabled = true;
        const res = await AuthModule.register({ name: order.customer.name, email: order.customer.email, phone: order.customer.phone, password: pass });
        btn.disabled = false;
        if (res.ok && res.pendingConfirmation) {
          Ui.closeModal(); Notify.info('Cuenta creada. Revisa tu correo para confirmarla.');
        } else if (res.ok) {
  ...
  async function start() { await resetState(); render(); }
