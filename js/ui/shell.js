/**
 * shell.js — construcción de la interfaz persistente (header/footer/
 * drawers/modal/búsqueda). Extraído de js/ui.js en la reorganización
 * arquitectónica; la tarjeta de producto vive en
 * js/ui/components/productCard.js — este archivo compone ambas piezas
 * en un único facade `Ui`, que es el global que el resto de la app ya
 * consume (docenas de call sites: `Ui.productGrid`, `Ui.openModal`,
 * etc.) — no se reparte ese global en pedazos para no perseguir cada
...
const Ui = (() => {
    ...
    el.innerHTML = `
      <div class="wrap">
        ...
        <div class="header-actions">
          <button class="icon-btn" id="btnCurrency" aria-label="Cambiar moneda" title="Cambiar moneda" style="font-family:var(--f-mono);font-size:11px;font-weight:700;width:auto;height:32px;padding:0 10px;border-radius:999px;border:1px solid var(--c-border);">${CurrencyService.getCurrency()}</button>
  ...
  function wireGlobalEvents() {
    document.body.addEventListener('click', (e) => {
      if (e.target.closest('#btnCurrency')) {
        CurrencyService.setCurrency(CurrencyService.getCurrency() === 'COP' ? 'USD' : 'COP');
      }
    ...
    // Cambio de moneda: es solo presentación (ver currencyService.js)
    // — nunca toca Store.state.products/cart/orders, solo vuelve a
    // pintar lo que ya está en pantalla con el nuevo formato.
    Store.on('currency:changed', (code) => {
      const btn = Utils.qs('#btnCurrency');
      if (btn) btn.textContent = code;
      Router.resolve();
      if (Utils.qs('#cartDrawer').classList.contains('open')) CartModule.renderDrawer();
    });
  }
  ...
  return {
    ...
    badgesHtml: UiProductCard.badgesHtml, productCard: UiProductCard.productCard, productGrid: UiProductCard.productGrid,
